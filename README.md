...
- **Cron is best-effort, and the drift is real.** GitHub queues scheduled runs and
  drops them under load, so "three a day, four hours apart" is the intent, not a
  guarantee. Measured here: the 17:23 UTC slot started at 18:27 UTC, an hour and
  four minutes late, and still did its work correctly. The slots deliberately sit
  at odd minutes rather than on the hour, which is when every other scheduled
  workflow on the platform wakes up. A late or missed slot costs one task, not the
  project: the next run picks up exactly where this one left off.
